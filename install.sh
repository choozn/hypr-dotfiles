...
sudo pacman --noconfirm --needed -S brightnessctl mako grim slurp man-db xclip wl-clipboard htop powertop fzf fd ffmpeg mpc mpd networkmanager network-manager-applet bluez bluetui blueman systemctl-tui mate-polkit helvum tmux viewnior wireguard-tools wget xarchiver zip unzip unrar 7zip openvpn ufw ranger jq wev thunar gvfs thunar-volman thunar-archive-plugin nvm fastfetch git-absorb ripgrep || { echo "[!] Failed to install dependency packages. Exiting."; exit 1; }
echo ""
