...
sudo pacman --noconfirm --needed -S aquamarine hyprland hyprlang hyprcursor hyprutils hyprgraphics hypridle hyprlock hyprpicker hyprwayland-scanner waybar swaybg wayland-utils qt5-wayland qt6-wayland xdg-desktop-portal-hyprland pipewire || { echo "[!] Failed to install Hyprland packages. Exiting."; exit 1; }
...
sudo pacman --noconfirm --needed -S brightnessctl mako grim slurp man-db xclip wl-clipboard htop powertop fzf fd ffmpeg mpc mpd networkmanager bluez bluetui systemctl-tui mate-polkit pulseaudio pulsemixer tmux viewnior wireguard-tools xarchiver zip unzip unrar 7zip openvpn ranger || { echo "[!] Failed to install dependency packages. Exiting."; exit 1; }
